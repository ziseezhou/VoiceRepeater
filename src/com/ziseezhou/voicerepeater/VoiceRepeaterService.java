/*************************************************************************
FILENAME VoiceRepeaterService.java

COPYRIGHT (c) 2013

DESCRIPTION
This file define VoiceRepeaterService class
...
public class VoiceRepeaterService extends Service {
	...
	public static final String PLAYSTATE_CHANGED = "com.ziseezhou.voicerepeater.playstatechanged";
    public static final String META_CHANGED = "com.ziseezhou.voicerepeater.metachanged";
    public static final String QUEUE_CHANGED = "com.ziseezhou.voicerepeater.queuechanged";
    public static final String REFRESH = "com.ziseezhou.voicerepeater.refresh";
    public static final String EXIT = "com.ziseezhou.voicerepeater.exit";
    
    public static final String SERVICECMD = "com.ziseezhou.voicerepeater.servicecommand";
    ...
    public static final String PREVIOUS_ACTION = "com.ziseezhou.voicerepeater.servicecommand.previous";
    public static final String NEXT_ACTION = "com.ziseezhou.voicerepeater.servicecommand.next";
    ...
    private int mSeekCounter = 0;
    
    // simple backward from media button
    private void scanBackward(int repcnt) {
        try {
            long nowpos = position();
            if (nowpos == -1) {
                // not ready for action
                Log.v(TAG, ">>> scanBackward pos=-1");
                return;
            }
    ...
    // simple forward from media button
    private void scanForward(int repcnt) {
        try {
            long nowpos = position();
            if (nowpos == -1) {
                // not ready for action
                Log.v(TAG, ">>> scanForward pos=-1");
                return;
            }
 ...
 // insert the list of songs at the specified position in the playlist
    ...
    public void play() {
        ...
        if (mPlayer.isInitialized()) {
            ...
            Log.e(TAG, "Request play, but playlist is null.");
        }
    }
    
    private void initNotification() {
        Log.v(TAG, ">>> initNotification()");
        RemoteViews views = new RemoteViews(getPackageName(), R.layout.statusbar);
        
        ComponentName localComponentName = new ComponentName(getPackageName(), VoiceRepeaterService.class.getName());
        Intent localIntent = new Intent(SERVICECMD);
        localIntent.setComponent(localComponentName);

        //views.setImageViewResource(R.id.status_play_pause, R.drawable.stat_pause);
        
        localIntent.putExtra(CMDNAME, CMDTOGGLEPAUSE);
        views.setOnClickPendingIntent(R.id.status_play_pause, PendingIntent.getService(getApplicationContext(), 1, localIntent, PendingIntent.FLAG_UPDATE_CURRENT));
        localIntent.putExtra(CMDNAME, CMDREWIND);
        views.setOnClickPendingIntent(R.id.status_next, PendingIntent.getService(getApplicationContext(), 2, localIntent, PendingIntent.FLAG_UPDATE_CURRENT));
        localIntent.putExtra(CMDNAME, CMDSTOP);
        views.setOnClickPendingIntent(R.id.status_stop, PendingIntent.getService(getApplicationContext(), 3, localIntent, PendingIntent.FLAG_UPDATE_CURRENT));
    ...
    private void updateNotification() {
        Log.v(TAG, ">>> updateNotification() statusbar="+mStatus);
        if (null != mStatus) {
            RemoteViews views = mStatus.contentView;
            
            if (isPlaying()) {
                views.setImageViewResource(R.id.status_play_pause, R.drawable.stat_pause);
            } else {
                views.setImageViewResource(R.id.status_play_pause, R.drawable.stat_play);
            }

            ((android.app.NotificationManager)getSystemService("notification")).notify(PLAYBACKSERVICE_STATUS, mStatus);
        }
    }
    ...
    private class MultiPlayer {
        ...
        public MultiPlayer() {
            mMediaPlayer.setWakeMode(VoiceRepeaterService.this, PowerManager.PARTIAL_WAKE_LOCK);
        }
        ...
        public void setDataSource(String path) {
            try {
                ...
                if (path.startsWith("content://")) {
                    mMediaPlayer.setDataSource(VoiceRepeaterService.this, Uri.parse(path));
                } else {
     ...
     */
    static class ServiceStub extends IVoiceRepeaterService.Stub {
        WeakReference<VoiceRepeaterService> mService;
        
        ServiceStub(VoiceRepeaterService service) {
            mService = new WeakReference<VoiceRepeaterService>(service);
        }
